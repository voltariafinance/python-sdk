...
Install from PyPI (published as `voltaria-sdk`, imported as `voltaria`):

```bash
pip install voltaria-sdk
```
...
Pass `base_url=` or `environment=` to override routing. An async client
(`AsyncVoltaria`) is also available with the same interface.
